...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_emb', action='store_true', help='Train word embedding.')
    ...
    for i in range(300):
        print
        'Epoch %d @ %s' % (i + 1, datetime.datetime.now())
        print
        ' Loss = %s' % epoch_train(model, optimizer, BATCH_SIZE, sql_data, table_data, schemas, TRAIN_ENTRY)
        train_tot_acc, train_bkd_acc = epoch_acc(model, BATCH_SIZE, sql_data, table_data, schemas, TRAIN_ENTRY,
                                                 train_flag=True)
        print
        ' Train acc_qm: %s' % train_tot_acc
        print
        ' Breakdown results: sel: %s, cond: %s, group: %s, order: %s' \
